...
set :keep_releases, 5
set :application, "SERVER TEST"
set :repository,  "git@github.com:dangluan/servertest.git"
set :scm, :git
...
namespace :deploy do
  ...
  task :restart, :roles => :web do
    if remote_file_exists?("#{shared_path}/pids/servertest.pid")
      run "kill -s QUIT `cat #{shared_path}/pids/servertest.pid`"
    end
